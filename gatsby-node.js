...
exports.onCreateNode = ({ node, getNode, actions }) => {
  ...
  if (node.internal.type === `MarkdownRemark`) {
    var sha = 'unknown';
    try {
      sha = execSync(`git log -n 1 --pretty=format:%H -- ${node.fileAbsolutePath}`, {stdio: ['ignore', 'pipe', 'ignore']})
      sha = sha.toString()
    } catch (e) {}
...
exports.createPages = ({ graphql, actions }) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    `
      ...
      const numPages = Math.ceil((posts.length - PostsOnIndex) / postsPerPage)
