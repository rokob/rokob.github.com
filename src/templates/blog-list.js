...
const BlogList = ({ pageContext , data }) => {
  ...
  <Layout>
    ...
    <Pagination numPages={numPages} pageNumber={pageNumber} prev={prev} next={next} />
  </Layout>
  )
}
...
const Pagination = ({numPages, pageNumber, prev, next}) => {
  ...
  if (numPages <= showAllLimit) {
    ...
    const leftContext = Math.min(Math.max(pageNumber - context + 1, 1), context);
    Array.from({length: leftContext}).forEach((_, i) => {
      const to = pageNumber - (context - i)
      const first = (pageNumber - context <= 1) && to < 2
      pages.push({key: key++, to: to < 2 ? `/` : `/blog/${to}`, name: `${to < 2 ? 1 : to}`, first: first})
    })
    ...
    if (pageNumber + context < numPages) {
      ...
      pages.push({key: key, to: `/blog/${numPages}`, name: `${numPages}`, first: false, last: true})
    }
  }
  return (
    <div>
    ...
    {pages.map(({ key, to, name, first, last }) => {
      if (to && first) {
        return <PaginationFirstLink key={key} to={to}>{name}</PaginationFirstLink>
      } else if (to && last) {
        return <PaginationLastLink key={key} to={to}>{name}</PaginationLastLink>
      } else if (to) {
        return <PaginationLink key={key} to={to}>{name}</PaginationLink>
      } else if (first) {
